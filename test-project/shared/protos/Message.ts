...
class Message$Type extends MessageType<Message> {
    ...
    internalBinaryWrite(message: Message, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        if (message.body.oneofKind === "requestMoneyFromMentor")
            RequestMoneyFromMentor.internalBinaryWrite(message.body.requestMoneyFromMentor, writer.tag(1, WireType.LengthDelimited).fork(), options).join();
        /* RequestMoneyFromMentorResponse requestMoneyFromMentorResponse = 2; */
        if (message.body.oneofKind === "requestMoneyFromMentorResponse")
            RequestMoneyFromMentorResponse.internalBinaryWrite(message.body.requestMoneyFromMentorResponse, writer.tag(2, WireType.LengthDelimited).fork(), options).join();
        /* Error error = 3; */
        if (message.body.oneofKind === "error")
            Error.internalBinaryWrite(message.body.error, writer.tag(3, WireType.LengthDelimited).fork(), options).join();
        /* SynchronizedState synchronizedState = 4; */
        if (message.body.oneofKind === "synchronizedState")
            SynchronizedState.internalBinaryWrite(message.body.synchronizedState, writer.tag(4, WireType.LengthDelimited).fork(), options).join();
